...
public class PBPacketHandler implements IPacketHandler
{
	...
	public void onPacketData(INetworkManager manager, Packet250CustomPayload packet, Player player) 
	{
		...
		if(packet.data.length  == 1){
			handlTinyPBPacket(packet, player);
			return;
		}
		int id = bis.readInt();
		int i = bis.readInt();
	...
	private void handlTinyPBPacket(Packet250CustomPayload packet, Player player) {
		byte tinyid = packet.data[0];
		if(!(player instanceof EntityPlayerMP))
			return;
		switch(tinyid){
		case 1: completeEmptyOfMatrix((EntityPlayerMP) player);
			break;
		case 2: ((ContainerProjectBench)(((EntityPlayerMP) player).openContainer)).writePlanToNBT();
			break;
		}
	}
	...
	public static void completeEmptyOfMatrix(EntityPlayerMP thePlayer) {
        ...
        for(int i = 0; i < 9; i++){
        	thePlayer.openContainer.transferStackInSlot(thePlayer, i + 2);
        }
