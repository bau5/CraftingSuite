...
/**
 * This is a provided class by NEI that has been adapted to work
 * with the inventories from Crafting Suite. I, _bau5, have edited
 * the relevant portions of the original class so that it may 
 * work with aspects of my mod. The relevant edits have been
 * marked below and should be noticeable. 
 * 
...
{    
    ...
    public void overlayRecipe(GuiContainer gui, IRecipeHandler recipe, int recipeIndex, boolean shift)
    {
    	...
    	boolean flag = ((ContainerBase)gui.inventorySlots).getInventoryModifier() == EnumInventoryModifier.PLAN;
    	if(result != null && flag){
    		ItemStack stackResult = result.item;
    		if(stackResult != null){
    			Slot slotWithPlan = null;
    			Slot planSlot = null;
    			for(Slot slot : (ArrayList<Slot>)gui.inventorySlots.inventorySlots){
    				if(slot.getHasStack() && slot.getStack().itemID == CraftingSuite.planItem.itemID && slot.getStack().getItemDamage() == 1){
    					ItemStack stackInSlot = slot.getStack();
    					if(stackInSlot.hasTagCompound()){
	    					ItemStack resultFromPlan = ItemStack.loadItemStackFromNBT((NBTTagCompound)stackInSlot.stackTagCompound.getTag("Result"));
	    					if(ItemStack.areItemStacksEqual(resultFromPlan, stackResult))
    						slotWithPlan = slot;
    					}
    				}
    				...
    				if(planSlot != null && slotWithPlan != null)
    					break;
    			}
    			if(slotWithPlan != null && planSlot != null && planSlot != slotWithPlan){
    				if(planSlot.getHasStack())
    					FastTransferManager.clickSlot(gui, planSlot.slotNumber, 0, 1);
    				FastTransferManager.clickSlot(gui, slotWithPlan.slotNumber, 0, 1);
    				usedPlan = true;
    			}
    		}
    	}
        ...
        if(usedPlan && quantity != 0){
        	for(IngredientDistribution distrib : assignedIngredients){
                ...
                {
                    if(!slot.getHasStack() || !(slot.inventory instanceof InventoryPlayer || slot.inventory instanceof TileEntityProjectBench))
                        continue;
                    ItemStack slotStack = slot.getStack();
                    if(ItemHelper.checkItemMatch(pstack, slotStack)){
